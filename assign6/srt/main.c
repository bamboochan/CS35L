...
enum { z = 1 };
/* color array */
float Result[width][height][3];

typedef struct my_bubble
{
  int m_nthread;
  int m_nthreads;
  //float* m_result[height][3];
  scene_t* m_scene;
} bgarg;

void * threadfn(void* arg)
{
  bgarg* bubbla = (bgarg*) arg;
    scene_t scene = *bubbla->m_scene;
    int nth = bubbla->m_nthread;
    int nthreads = bubbla->m_nthreads;
    //printf("%d %d\n",nth,nthreads);
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px=nth; px<width; px+=nthreads )
    {
	    ...
	    //printf("finished tracing %d\n",nth);
             ...
             */
	    for(int i=0;i<3;i++)
	      Result[px][py][i]=scaled_color[i];
	    //	    printf("finished saving %d\n",nth);
	}
...
main( int argc, char **argv )
{
    ...
    //initializa thread array
    pthread_t* threadarr = (pthread_t*)malloc(sizeof(pthread_t)*nthreads);
    bgarg* argarr = (bgarg*) malloc(sizeof(bgarg)*nthreads);
    if (threadarr==NULL ||argarr==NULL)
      {
    ...
    for (int i=0;i<nthreads;i++)
      {
	argarr[i].m_scene = &scene;
	//argarr[i].m_result = Result;
	argarr[i].m_nthread = i;
	argarr[i].m_nthreads = nthreads;
      }
    ...
    for(int t = 0; t < nthreads; t++)
      {
	int ret = pthread_create(&threadarr[t], NULL,threadfn,&argarr[t]);
	if(ret)
	  {
    ...
    for(int t = 0; t < nthreads; t++)
      {
	int ret = pthread_join(threadarr[t], NULL);
	if(ret)
	  {
